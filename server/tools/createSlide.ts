...
async function generateSingleImage(
  ...
  while (attempt <= maxRetries) {
    ...
    if (enableQA && attempt <= maxRetries) {
      ...
      if (correction) {
        console.log(`[generateSingleImage] ⚠️ QA found issues: ${correction}`);
        console.log(`[generateSingleImage] 🔄 Retrying with corrections...`);
        lastPrompt = `${prompt}\n\n**CRITICAL CORRECTION FROM QA:** You made spelling errors. Please regenerate and fix: ${correction}`;
        continue; // Retry
      } else {
        console.log(`[generateSingleImage] ✅ QA passed - text is accurate`);
        break; // Success!
      }
    } else {
