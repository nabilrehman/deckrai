...
const Navbar = ({ currentView, setView, credits }: { currentView: ViewState, setView: (v: ViewState) => void, credits: number }) => {
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 backdrop-blur-md border-b border-slate-200/50 h-14' : 'bg-transparent h-20'}`}>
      <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
           {currentView === 'HOME' && (
             <button onClick={() => setView('DASHBOARD')} className="text-sm font-medium text-slate-500 hover:text-slate-900 mr-2">
               Sign In
             </button>
           )}
           {currentView === 'DASHBOARD' && (
              <div className="flex items-center gap-2 px-3 py-1 bg-slate-100 rounded-full text-xs font-bold text-slate-700 border border-slate-200">
                 <CreditCard size={12} /> {credits}
              </div>
           )}
           <Button variant="primary" onClick={() => setView('DASHBOARD')}>
              {currentView === 'HOME' ? 'Start Free Trial' : 'Dashboard'}
           </Button>
        </div>
      </div>
    </nav>
  ...
  if (view === 'DASHBOARD') {
     return (
       <div className="min-h-screen bg-[#FAFAFA] font-sans">
          <Navbar currentView={view} setView={setView} credits={credits} />
          <Dashboard credits={credits} onAddCredits={(n) => setCredits(c => c + n)} />
       </div>
     );
  }
      ...
      <section className="relative pt-32 pb-20 px-6 overflow-hidden">
         ...
         <div className="max-w-6xl mx-auto text-center relative z-10">
            ...
            <motion.h1 
               ...
               className="text-6xl md:text-8xl font-bold tracking-tighter text-slate-900 mb-8 leading-[0.9] relative"
            >
               <span className="relative inline-block">
                  Stop Sending
                  <span className="absolute -top-4 -right-6 text-[10px] font-normal text-slate-400 font-mono tracking-widest hidden md:block">SYS_GEN_01</span>
               </span>
               <br/>
               ...
               <span className="text-4xl md:text-6xl text-slate-800">
                   We Adapt Your Best Slides.
               </span>
            </motion.h1>

            <motion.p 
               initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
               className="text-xl text-slate-500 max-w-2xl mx-auto mb-12 font-light leading-relaxed"
            >
            ...
            <motion.div 
               ...
               className="max-w-xl mx-auto mb-24 relative z-20"
            >
               <div className="relative group">
                  <div className="absolute -inset-1 bg-gradient-to-r from-blue-500 via-purple-500 to-blue-500 rounded-2xl opacity-20 blur-lg group-hover:opacity-40 transition duration-1000" />
                  <div className="relative bg-white rounded-xl shadow-2xl flex items-center p-2 ring-1 ring-slate-200/50">
                     <div className="pl-4 text-slate-400"><Command size={20} /></div>
                     <input 
                        type="text" 
                        value={heroInput}
                        onChange={(e) => setHeroInput(e.target.value)}
                        onKeyDown={(e) => e.key === 'Enter' && handleHeroGenerate()}
                        className="flex-1 p-4 outline-none text-slate-900 placeholder:text-slate-400 bg-transparent"
                        placeholder="tailor our sales deck for the upcoming meeting with Atlassion"
                     />
                     <Button onClick={handleHeroGenerate} className="shadow-none">
                        Generate <ArrowRight size={16} />
                     </Button>
                  </div>
                  {/* Input HUD Details */}
                  <div className="absolute -bottom-6 left-0 text-[9px] font-mono text-slate-300 uppercase tracking-widest flex gap-4">
                     <span>Latency: 12ms</span>
                     <span>Model: Gemini 2.5</span>
                  </div>
               </div>
               <div className="mt-8 flex justify-center gap-6 text-xs font-medium text-slate-400">
                  <span className="flex items-center gap-1.5"><ShieldCheck size={12} className="text-green-500" /> Brand Guardrails</span>
                  <span className="flex items-center gap-1.5"><LayoutTemplate size={12} className="text-green-500" /> Uses Your Layouts</span>
                  <span className="flex items-center gap-1.5"><Check size={12} className="text-green-500" /> SOC2 Compliant</span>
               </div>
            </motion.div>
                           ...
                           {[
                              ...
                              >
                                 <div 
                                    ...
                                    style={{ transform: `rotate(${i * 72}deg) translateX(140px) rotate(-${i * 72}deg) rotate(-${360}deg)` }} // Counter-rotate content
                                 >
